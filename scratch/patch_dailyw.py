...
def update_dailyw():
    ...
    # Just in case it was already replaced or has slightly different spacing, we can use regex if it fails.

    # 4. Modify clearForm, openDwModal, saveDw, etc.
    # Replace openDwModal
  ...
  setTimeout(()=>document.querySelector('.row-act')?.focus(),50); 
    ...
    if "setTimeout(()=>document.getElementById('dw-activity').focus(),50);" in html:
  ...
  try{
    ...
    payload.addedBy = u.name || 'Employee';
  ...
  try{
    ...
    // We get total hours for the day so far
    let totalDayMins = calcTotalHours();
...
function exportToExcel() {
  ...
  // Calculate total hours per day to display correctly in Excel
...
"""
    if "function saveDw(){" in html and "function exportToExcel()" not in html:
