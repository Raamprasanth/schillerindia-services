...
const fqcNonsaleableSchema = new mongoose.Schema(
  {
    ...
    unitDetails: {
      type: String,
      enum: ['Demo', 'Sales Stock', 'Warranty Failure', 'DOA', 'Sales Return'],
      required: true,
    },
  ...
  {
    timestamps: true,
    collection: 'fqcnonsaleable',
  }
);

// Indexes for common query patterns
fqcNonsaleableSchema.index({ status: 1 });
fqcNonsaleableSchema.index({ division: 1 });
fqcNonsaleableSchema.index({ model: 1 });
fqcNonsaleableSchema.index({ modelSn: 1 });
fqcNonsaleableSchema.index({ engineer: 1 });
fqcNonsaleableSchema.index({ fqcInDate: 1 });
fqcNonsaleableSchema.index({ scInDate: 1 });
fqcNonsaleableSchema.index({ createdBy: 1 });

module.exports =
  mongoose.models.FqcNonsaleable ||
  mongoose.model('FqcNonsaleable', fqcNonsaleableSchema);
