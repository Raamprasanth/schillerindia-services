...
const ptBirSchema = new mongoose.Schema(
  {
  ...
  {
    timestamps: true,
    collection: 'ptbirrecords',
  }
);

ptBirSchema.index({ status: 1 });
ptBirSchema.index({ division: 1 });
ptBirSchema.index({ model: 1 });
ptBirSchema.index({ birRef: 1 });
ptBirSchema.index({ unitInwardDate: 1 });
ptBirSchema.index({ createdBy: 1 });

module.exports = mongoose.models.PtBir || mongoose.model('PtBir', ptBirSchema);
