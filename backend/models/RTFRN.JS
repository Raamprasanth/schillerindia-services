// models/RTFRN.js
// ─────────────────────────────────────────────────────────
//  Repair Tracking FRN (RT FRN) — stores PFRN / OB / UR
//  repair activity records submitted by employees.
// ─────────────────────────────────────────────────────────

const mongoose = require('mongoose');

const RTFRNSchema = new mongoose.Schema(
  {
    // ── Core identity ────────────────────────────────────
    entryDate: {
  ...
  },
  {
    ...
    collection:  'rtfrns',    // explicit collection name in MongoDB
  }
);

// ── Indexes for fast queries ──────────────────────────────
RTFRNSchema.index({ scRefNo:   1 });
RTFRNSchema.index({ defGirNo:  1 });
RTFRNSchema.index({ division:  1 });
RTFRNSchema.index({ status:    1 });
RTFRNSchema.index({ entryDate: -1 });
RTFRNSchema.index({ submittedBy: 1 });

module.exports = mongoose.model('RTFRN', RTFRNSchema);
