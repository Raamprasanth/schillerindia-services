...
const aClosedBirSchema = new mongoose.Schema(
  {
    ...
    finalStatus: { type: String, trim: true, default: 'Closed' },
    createdBy: { type: mongoose.Schema.Types.ObjectId, ref: 'Admin' },
    updatedBy: { type: mongoose.Schema.Types.ObjectId, ref: 'Admin' },
  },
  {
    timestamps: true,
    collection: 'adminclosedbirrecords',
  }
);

aClosedBirSchema.index({ status: 1 });
aClosedBirSchema.index({ finalStatus: 1 });
aClosedBirSchema.index({ division: 1 });
aClosedBirSchema.index({ model: 1 });
aClosedBirSchema.index({ birRef: 1 });
aClosedBirSchema.index({ unitInwardDate: 1 });
aClosedBirSchema.index({ approvedDate: 1 });
aClosedBirSchema.index({ createdBy: 1 });

module.exports = mongoose.models.AClosedBir || mongoose.model('AClosedBir', aClosedBirSchema);
