...
//
//  GET    /api/rtrr                → all records (admin)
//  GET    /api/rtrr/employee       → records for logged-in employee
//  POST   /api/rtrr                → create new record
//  PUT    /api/rtrr/:id            → update record
//  DELETE /api/rtrr/:id            → delete record
// ─────────────────────────────────────────────────────────
...
// ══════════════════════════════════════════════════════════
//  GET /api/rtrr  — all records (admin only)
// ══════════════════════════════════════════════════════════
router.get('/', protect, adminOnly, async (req, res) => {
  try {
    ...
    const records = await Rtrr.find(filter)
      .sort({ entryDate: -1 })
      .limit(parseInt(limit))
      .lean();
    return res.json(await attachActualDivisions(records));
  } catch (err) {
    console.error('Rtrr GET all error:', err.message);
    return res.status(500).json({ success:false, message: err.message });
  }
});

// ══════════════════════════════════════════════════════════
//  GET /api/rtrr/employee  — records for logged-in user
...
router.get('/employee', protect, async (req, res) => {
  try {
    ...
    console.error('Rtrr GET employee error:', err.message);
    return res.status(500).json({ success:false, message: err.message });
  }
});

// ══════════════════════════════════════════════════════════
//  GET /api/rtrr/:id  — single record
// ══════════════════════════════════════════════════════════
router.get('/:id', protect, async (req, res) => {
  try {
    const record = await Rtrr.findById(req.params.id).lean();
...
// ══════════════════════════════════════════════════════════
//  POST /api/rtrr  — create new Re-repair List record
// ══════════════════════════════════════════════════════════
router.post('/', protect, async (req, res) => {
  try {
    ...
    console.error('Rtrr POST error:', err.message);
...
// ══════════════════════════════════════════════════════════
//  PUT /api/rtrr/:id  — update Re-repair List record
// ══════════════════════════════════════════════════════════
router.put('/:id', protect, async (req, res) => {
  try {
    const existing = await Rtrr.findById(req.params.id).lean();
    ...
    const updated = await Rtrr.findByIdAndUpdate(
      req.params.id,
      updates,
    ...
    if (targetEmpFrnId) {
      try {
        ...
        console.error('Rtrr → EmpFRN sync failed:', empSyncErr.message);
      }
    }
    ...
    console.error('Rtrr PUT error:', err.message);
    return res.status(500).json({ success:false, message: err.message });
  }
});

// ══════════════════════════════════════════════════════════
//  DELETE /api/rtrr/:id  — delete record (admin only)
// ══════════════════════════════════════════════════════════
router.delete('/:id', protect, async (req, res) => {
  try {
    ...
    const deleted = await Rtrr.findByIdAndDelete(req.params.id);
    return res.json({ success:true, message:`Record "${deleted.scRefNo}" deleted.` });
  } catch (err) {
    console.error('Rtrr DELETE error:', err.message);
    return res.status(500).json({ success:false, message: err.message });
  }
});
