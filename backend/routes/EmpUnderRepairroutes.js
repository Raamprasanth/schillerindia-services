...
router.post('/', protect, async (req, res) => {
  try {
    ...
    const doc = await UnderRepair.create({
      ...
      scRno, scEng, frnNo, region,
      eng:          engineer || req.body.eng || '',
      engineer:     engineer || req.body.eng || '',
...
router.put('/:id/update', protect, async (req, res) => {
  try {
    ...
    const update = {
      ...
      repBrd:       repBrd      || '',
      repBrdDate:   repBrd      || req.body.repBrdDate || '',
      finalRemarks,
      ...
      await Service.findByIdAndUpdate(
        doc.serviceId,
        {
          $set: {
            ...
            repBrd:       doc.repBrd || doc.repBrdDate || '',
            repBrdDate:   doc.repBrd || doc.repBrdDate || '',
            finalRemarks: doc.finalRemarks || '',
            techRemarks:  doc.techRemarks || '',
            components:   doc.components || '',
            typeWork:     doc.typeWork || doc.typeOfWork || '',
