...
function replaceSection(prefix, functionCall) {
  ...
  // Find where the marker is that contains selectId
  let startIndex = 0;
  while (true) {
    ...
    // We know it ends after the button with the given functionCall
    let btnStart = content.indexOf(\`onclick="\${functionCall}"\`, blockStart);
    if (btnStart !== -1) {
      // Check if this block actually has our selectId
...
// Re-run for the remaining 3 tabs
