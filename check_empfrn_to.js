...
async function run() {
  ...
  try {
    ...
    for (const frn of escalatedFrns) {
      const matchCount = await Todr.countDocuments({ sourceId: String(frn._id) });
      console.log(`FRN ID: ${frn._id}, FRN No: ${frn.frnNo}, queuedAt: ${frn.toEscalationQueuedAt}, matches in Todr: ${matchCount}`);
    }
